...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		File dir = new File("C:/TempTest/images");
		File file1 = new File("C:/TempTest/file1.txt");
		File file2 = new File("C:/TempTest/file2.txt");
		File file3 = new File("C:\\TempTest\\file3.txt"); // 문자열에서는 \\로 이스케이프 필요
		
		// 해당 경로에 실제 파일이나 폴더가 있는지 검사 => 없음
		// 존재하지 않으면 생성
		// 폴더 생성
		...
		File dirTest = new File("C:/TempTest/test/videos");
//		if (!dirTest.exists()) dirTest.mkdir(); // 경로상 맨뒤 폴더만 생성(앞에 상위 폴더가 있어야 함)
		...
		dirTest = new File("C:/TempTest/test");
		if (dirTest.delete()) {
		...
		System.out.println("dir는 폴더? " + dir.isDirectory()); // true
		System.out.println("dir는 파일? " + dir.isFile()); // false
		System.out.println("file1는 폴더? " + file1.isDirectory()); // false
		System.out.println("file1는 파일? " + file1.isFile()); // true
		
		// 상위(부모) 폴더 확인
		// 파일이나 폴더가 속한 상위(부모) 폴더의 경로를 문자열로 리턴함
		System.out.println(dir.getParent());
		// 상위(부모) 폴더를 File 객체로 만든 후 리턴함
		...
		if (temp.exists() && temp.isDirectory()) {
			...
			for (File file : contents) {
				...
				System.out.print(sdf.format(new Date(file.lastModified()))); // 밀리초
