...
//		Calendar now = new Calendar(); // 추상 클래스이기 때문에 new 연산자로 직접 객체 생성 불가
		Calendar now = Calendar.getInstance(); // static 메소드.. Calendar의 하위(자식) 객체를 리턴
		
		int year = now.get(Calendar.YEAR); // 연도 .. Calendar.YEAR = 1
		...
		LocalDateTime minusDays = now2.minusDays(3);
		System.out.println("3일 전: " + minusDays);
		...
		// 날짜/시간 차이 구하기: Period.between() 날짜차이, Duration.between() 시간차이
		// 날짜 차이
